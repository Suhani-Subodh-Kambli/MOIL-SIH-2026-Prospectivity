...
export default function App() {
  ...
  useEffect(() => {
    if (!token || page !== 'exploration' || grid) return;
    api.grid().then((data) => { if (data?.available) setGrid(data); }).catch(() => {});
  }, [token, page, grid]);
    ...
    <div className="shell">
      ...
      <main>
        ...
        <div className="content">
          ...
          {page === 'exploration' && <Exploration targets={targets} grid={grid} zones={zones} saveActivity={saveActivity} />}
...
function Exploration({ targets, grid, zones, saveActivity }) {
  const [mode, setMode] = useState('point');
  const [selection, setSelection] = useState(null);
  const [analysis, setAnalysis] = useState(null);
  const [selectedZone, setSelectedZone] = useState(null);

  const points = useMemo(() => (grid?.features || []).map((f, i) => {
    const g = f?.geometry;
    const p = f?.properties || {};
    if (g?.type === 'Point' && Array.isArray(g.coordinates)) {
      return { i, lat: Number(g.coordinates[1]), lon: Number(g.coordinates[0]), score: Number(p.prospectivity_score ?? p.score) };
    }
    return null;
  }).filter(p => p && Number.isFinite(p.lat) && Number.isFinite(p.lon) && Number.isFinite(p.score)), [grid, targets]);

  const analyze = () => {
    if (!selection) return;
    let result = null;
    if (selection.kind === 'point') {
      const nearest = nearestPoint(points, selection.lat, selection.lon);
      result = {
        kind: 'point', lat: selection.lat, lon: selection.lon,
        score: nearest?.score, distanceKm: nearest ? distanceKm(selection.lat, selection.lon, nearest.lat, nearest.lon) : null,
        source: 'full detailed prediction grid'
      };
    } else {
      const inside = selection.kind === 'rectangle'
        ? points.filter(p => insideRect(p, selection.bounds))
        : points.filter(p => insidePolygon(p.lat, p.lon, selection.path));
      result = areaStats(inside, selection.kind);
    }
    setAnalysis(result);
    setSelectedZone(null);
    saveActivity({ type: 'Exploration analysis', detail: result.kind === 'point' ? `Point ${result.lat.toFixed(4)}, ${result.lon.toFixed(4)}` : `${result.kind} analysis: ${result.count} cells` });
  };

  const clear = () => { setSelection(null); setAnalysis(null); setSelectedZone(null); };
  const chooseMode = (next) => { setMode(next); setSelection(null); setAnalysis(null); setSelectedZone(null); };
  const zoneSelect = (z) => { setSelectedZone(zoneFrom(z)); setAnalysis(null); setSelection(null); };
  ...
  return <>
    ...
    <section className="exploreToolbar panel">
      <div className="toolbarIntro">
        <div><small>ANALYSIS MODE</small><b>Select an area, then run analysis</b></div>
      ...
      <div className="modeButtons">
        ...
        ].map(([v,t]) => <button key={v} className={mode === v ? 'mode active' : 'mode'} onClick={() => chooseMode(v)}>{t}</button>)}
        <button className="gold analyzeBtn" disabled={!selection || !!analysis} onClick={analyze}>{analysis ? 'Analysis complete ✓' : 'Analyze selection →'}</button>
      </div>
      <div className="toolbarHint">
        {mode === 'point' ? 'Click a location once to place a selection. Nothing is calculated until Analyze selection.' : mode === 'rectangle' ? 'Press and drag across the map to draw a rectangle. Release to set the area, then click Analyze selection.' : 'Press and drag around the area to freehand-draw a polygon. Release to set it, then click Analyze selection.'}
      </div>
      {selection && !analysis && <div className="pendingSelection"><span>SELECTION READY</span><b>{selection.kind === 'point' ? `${selection.lat.toFixed(4)}, ${selection.lon.toFixed(4)}` : `${selection.kind} drawn on map`}</b><em>Click Analyze selection</em></div>}
    </section>
    <div className="exploreLayout">
      <section className="panel mapPanel"><Head e="LIVE SPATIAL VIEW" t="India target geography"/><MapView targets={targets} grid={grid} zones={zones} occurrences={KNOWN_OCCURRENCES} mode={mode} selection={selection} locked={Boolean(analysis)} onSelectionChange={setSelection} onZoneSelect={zoneSelect}/></section>
      <section className="panel analysisPanel"><AnalysisPanel analysis={analysis} zone={selectedZone} gridAvailable={Boolean(grid?.features?.length)} />
...
function AnalysisPanel({ analysis, zone, gridAvailable }) {
  if (zone) return <div className="analysisBox"><div className="miniLabel">PRIORITY ZONE</div><h3>{zone.name}</h3><div className="scoreHero"><b>{zone.score.toFixed(1)}</b><span>mean prospectivity</span></div><div className="analysisMetrics"><Metric l="Maximum" v={Number.isFinite(zone.max) ? zone.max.toFixed(1) : '—'}/><Metric l="High cells" v={Number.isFinite(zone.high) ? zone.high.toLocaleString() : '—'}/><Metric l="High fraction" v={Number.isFinite(zone.fraction) ? `${(zone.fraction * 100).toFixed(1)}%` : '—'}/></div><div className="evidence"><b>Interpretation</b><p>This zone is part of the detailed India-wide screening layer. Use it to prioritize further geological investigation.</p></div></div>;
  if (!analysis) return <div className="empty analysisEmpty"><div className="crosshair">⌖</div><h3>Select an exploration area</h3><p>Choose point, rectangle or polygon mode, draw/select an area, then press <b>Analyze selection</b>.</p><div className="layerState">{gridAvailable ? '● Full detailed screening grid loaded' : '● Detailed score grid not loaded; point scores use the available target layer.'}</div></div>;
  if (analysis.kind === 'point') return <div className="analysisBox"><div className="miniLabel">POINT ANALYSIS</div><h3>Selected location</h3><div className="coords"><span>LATITUDE <b>{analysis.lat.toFixed(6)}</b></span><span>LONGITUDE <b>{analysis.lon.toFixed(6)}</b></span></div>{analysis.score != null ? <><div className="scoreHero"><b>{analysis.score.toFixed(1)}</b><span>prospectivity / 100</span></div><PriorityBadge score={analysis.score}/><div className="evidence"><b>Model evidence</b><p>Nearest modelled cell: {analysis.distanceKm != null ? `${analysis.distanceKm.toFixed(2)} km away.` : 'within the loaded prediction layer.'}</p><p>Source: {analysis.source}. Use this result to prioritize field investigation, not to establish a reserve.</p></div></> : <div className="empty smallEmpty"><h3>No modelled cell nearby</h3><p>The selected location is outside the loaded prediction layer.</p></div>}</div>;
  return <div className="analysisBox"><div className="miniLabel">AREA ANALYSIS</div><h3>{analysis.kind === 'rectangle' ? 'Selected rectangle' : 'Selected polygon'}</h3>{analysis.count ? <><div className="analysisMetrics four"><Metric l="Cells" v={analysis.count.toLocaleString()}/><Metric l="Mean score" v={analysis.mean.toFixed(1)}/><Metric l="Maximum" v={analysis.max.toFixed(1)}/><Metric l="High cells" v={analysis.high.toLocaleString()}/></div><div className="scoreBar"><span style={{ width: `${Math.min(100, Math.max(0, analysis.mean))}%` }}/></div><div className="areaHeadline"><b>{analysis.highFraction.toFixed(1)}%</b><span>of selected modelled cells are high-priority</span></div><div className="evidence"><b>Decision support</b><p>Prioritize field checks around the highest-scoring cells inside this area. Geological confirmation remains necessary.</p></div></> : <div className="empty smallEmpty"><h3>No modelled cells in this area</h3><p>Expand the selected area or move it over a detailed screening region.</p></div>}</div>;
}
...
function insideRect(p, bounds) { return p.lat >= bounds[0][0] && p.lat <= bounds[1][0] && p.lon >= bounds[0][1] && p.lon <= bounds[1][1]; }
...
function areaStats(points, kind) { const scores = points.map(p => p.score).filter(Number.isFinite); if (!scores.length) return { kind, count: 0, mean: 0, max: 0, high: 0, highFraction: 0 }; const mean = scores.reduce((a, b) => a + b, 0) / scores.length; const max = Math.max(...scores); const high = scores.filter(s => s >= 90).length; return { kind, count: scores.length, mean, max, high, highFraction: high / scores.length * 100 }; }
